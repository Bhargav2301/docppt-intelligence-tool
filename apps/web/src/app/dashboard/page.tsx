...
import { Plus, History, Clock, Presentation, Activity, RefreshCw, Eye, Trash2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  const fetchSessions = async () => {
    try {
      ...
      // Filter to only show PPT sessions in dashboard
      setSessions(data.filter(s => s.session_type === "ppt"));
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoading(false);
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[var(--text-secondary)] mt-1">Manage your presentation humanization sessions.</p>
        </div>
      ...
      <div className="flex justify-center w-full">
        <Link 
          href="/process/ppt"
          className="flex items-center gap-4 p-6 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] hover:border-[var(--accent)] hover:bg-[var(--bg-elevated)] transition-all group w-full max-w-xl"
        >
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)]">Clean and humanize a PowerPoint presentation</p>
          </div>
        </Link>
      </div>
        ...
        <div className="p-0">
          {isLoading && sessions.length === 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-[var(--text-secondary)]">No PPT sessions yet.</p>
              <p className="text-sm text-[var(--text-muted)] mt-1">Start by uploading a PPTX file above.</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs text-[var(--text-muted)]">
                        ...
                        {session.metrics?.total_slides && (
                          <>
                            <span>•</span>
                            <span>{session.metrics.total_slides} slides</span>
                          </>
                        )}
